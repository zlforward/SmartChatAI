    ...
    <div className={`min-h-screen ${theme === 'dark' ? 'bg-slate-900 text-white' : 'bg-white text-slate-900'}`}>
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="text-center mb-12">
          <h2 className="text-3xl font-bold mb-3">视频生成</h2>
          <p className="text-muted-foreground max-w-2xl mx-auto">
            智能视频创作与编辑，让创意无限可能
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className={`p-6 rounded-xl ${theme === 'dark' ? 'bg-slate-800' : 'bg-white'} shadow-lg`}>
            ...
            <div className="space-y-3">
              {['可灵', '即梦', 'VIDU'].map((model, index) => (
                <div
                  key={index}
                  onClick={() => setSelectedSubTool(model)}
                  className={`p-4 rounded-lg cursor-pointer transition-all ${
                    selectedSubTool === model
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      <Clapperboard className={`h-5 w-5 ${selectedSubTool === model ? 'text-white' : 'text-zhiliao-500'}`} />
